...
class AtlasHistoryPolicyTests(unittest.TestCase):
    def test_explicit_report_value_wins_over_legacy_override(self) -> None:
    ...
    def test_invalid_override_requires_reason_and_nonnegative_counts(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            path = Path(temp) / "overrides.json"
            path.write_text(
                json.dumps(
