...
class SampleCorpusReleaseWorkflowTests(unittest.TestCase):
    ...
    def test_draft_release_is_resolved_by_cli_database_id(self) -> None:
        text = WORKFLOW.read_text(encoding='utf-8')
        self.assertIn("'gh', 'release', 'view', tag", text)
        self.assertIn("'databaseId,tagName,targetCommitish,isDraft,assets'", text)
        self.assertIn('--json databaseId --jq .databaseId', text)
        self.assertIn('--json isDraft --jq .isDraft', text)
        self.assertNotIn('releases/tags/studio-sample-corpus-', text)
