...
const stateColor = computed(() => {
  ...
  if (status.value?.state === 'available' || status.value?.state === 'downloaded' || status.value?.state === 'offline-staged') return 'success'
...
<template>
  <div class="page-wrap">
    <PageHeader
      eyebrow="Update & Recovery Center"
      title="Verified upgrades with a way back"
      subtitle="Online and offline update packages are gated by active jobs, platform identity, hashes and recovery checkpoints. Restores run before SQLite opens."
      icon="mdi-update"
    />
    ...
    <v-row>
      <v-col cols="12" lg="7">
        <v-card class="glass pa-6 h-100">
          <div class="d-flex flex-wrap align-center justify-space-between ga-3 mb-5">
            ...
            <v-chip :color="stateColor" :prepend-icon="stateIcon" variant="tonal">
          ...
          <div class="d-flex flex-wrap ga-2">
            ...
            <v-btn
              ...
              @click="installOnline"
            >Back up & install</v-btn>
            <v-btn
              v-if="status?.releaseUrl"
              variant="tonal"
              prepend-icon="mdi-github"
              @click="window.open(status.releaseUrl, '_blank')"
            >Release page</v-btn>
          </div>
        </v-card>
      </v-col>
      ...
      <v-col cols="12" lg="5">
        <v-card class="glass pa-6 h-100">
          ...
          <PathField
            v-model="offlineManifest"
            kind="file"
            :extensions="['json']"
            label="Offline update manifest"
            hint="Stable manifests require an Ed25519 signature."
          />
          <PathField
            v-model="offlinePackage"
            kind="file"
            :extensions="['exe', 'msi', 'dmg', 'zip', 'AppImage', 'deb']"
            label="Matching installer package"
          />
          <div class="d-flex flex-wrap ga-2">
            <v-btn
              color="secondary"
              prepend-icon="mdi-package-down"
              :disabled="!offlineManifest || !offlinePackage"
              :loading="busy === 'offline'"
              @click="importOffline"
            >Verify & stage</v-btn>
            <v-btn
              v-if="status?.state === 'offline-staged'"
              color="success"
              prepend-icon="mdi-package-up"
              :loading="busy === 'open-offline'"
              @click="openOffline"
            >Back up & open</v-btn>
          </div>
        </v-card>
      ...
      <v-col cols="12" lg="4">
        <v-card class="glass pa-6 h-100">
          ...
          <div class="d-flex align-center ga-3 mt-2">
            <v-avatar :color="integrity?.ok ? 'success' : 'error'" variant="tonal">
              <v-icon :icon="integrity?.ok ? 'mdi-database-check-outline' : 'mdi-database-alert-outline'" />
            </v-avatar>
            <div>
      ...
      <v-col cols="12" lg="8">
        <v-card class="glass pa-6 h-100">
          <div class="d-flex flex-wrap align-start justify-space-between ga-3 mb-4">
            <div>
              <div class="text-overline text-secondary">Recovery checkpoints</div>
              <div class="text-h6 font-weight-bold">Hash-verified application state</div>
            </div>
          ...
          <v-list v-else bg-color="transparent" lines="three">
            <v-list-item v-for="backup in backups" :key="backup.id" :title="backup.reason">
              <template #prepend>
                <v-avatar :color="backup.verified ? 'success' : 'error'" variant="tonal">
                  <v-icon :icon="backup.verified ? 'mdi-shield-check-outline' : 'mdi-shield-alert-outline'" />
                </v-avatar>
              </template>
              <template #subtitle>
                <span>{{ backup.createdAt }} · v{{ backup.appVersion }} · {{ backup.fileCount }} files · {{ (backup.totalBytes / 1048576).toFixed(1) }} MiB</span>
              </template>
              <template #append>
                <div class="d-flex ga-1">
                  <v-btn icon="mdi-folder-open-outline" variant="text" color="secondary" @click="window.mel.revealPath(backup.manifestPath)" />
                  <v-btn
