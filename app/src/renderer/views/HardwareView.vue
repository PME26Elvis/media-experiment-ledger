...
const providers = computed(() => {
  const labels: Record<HardwareProviderKey, string> = {
  ...
  return (Object.keys(labels) as HardwareProviderKey[]).map(key => {
    ...
    return {
      key,
      label: labels[key],
      provider: support?.provider ?? 'unknown',
      available: support?.available === true,
    }
  })
})
const providerItems = computed(() => providers.value.map(item => ({
  title: `${item.label}${item.available ? '' : ' · unavailable'}`,
  value: item.key,
  props: { disabled: !item.available },
})))
...
async function runSelfTest() {
  ...
  try {
    ...
    notify(
      selfTest.value.passed ? 'Provider self-test passed.' : 'Provider self-test completed with a warning or failure.',
      selfTest.value.passed ? 'success' : 'warning',
    )
  } catch (error) {
    notify(error instanceof Error ? error.message : String(error), 'error')
  } finally { busy.value = '' }
...
onMounted(async () => {
  ;[preferences.value, snapshot.value] = await Promise.all([
    window.melHardware.preferences.get(),
    window.melHardware.snapshot(),
  ])
  if (!selectedAvailable.value) preferences.value.provider = 'cpu'
})
</script>
  ...
  <div class="page-wrap">
    ...
    <v-row class="mt-1">
      <v-col cols="12" lg="7">
        <v-card class="glass pa-6 h-100">
          ...
          <v-table density="comfortable" class="rounded-lg">
            <thead><tr><th>Provider</th><th>Runtime name</th><th>Status</th></tr></thead>
            <tbody><tr v-for="item in providers" :key="item.key"><td class="font-weight-medium">{{ item.label }}</td><td>{{ item.provider }}</td><td><v-chip :color="item.available ? 'success' : 'warning'" variant="tonal" size="small">{{ item.available ? 'Registered' : 'Unavailable' }}</v-chip></td></tr></tbody>
          </v-table>
      ...
      <v-col cols="12" lg="5">
        <v-card class="glass pa-6 h-100">
          ...
          <v-list v-if="snapshot?.gpuDevices.length" bg-color="transparent" lines="three">
            <v-list-item v-for="device in snapshot.gpuDevices" :key="device.id" prepend-icon="mdi-memory">
              <v-list-item-title>{{ device.name ?? `GPU adapter ${device.id}` }}</v-list-item-title>
              <v-list-item-subtitle>{{ device.vendor ?? device.driverVendor ?? 'Unknown vendor' }} · device {{ device.deviceId ?? 'unknown' }} · driver {{ device.driverVersion ?? 'unknown' }}</v-list-item-subtitle>
              <template #append><v-chip :color="device.active ? 'success' : 'default'" size="small" variant="tonal">{{ device.active ? 'Active' : 'Detected' }}</v-chip></template>
            </v-list-item>
          </v-list>
    ...
    <v-card class="glass pa-6 mt-6">
      ...
      <div class="d-flex flex-wrap ga-2">
        <v-btn color="primary" prepend-icon="mdi-content-save-outline" :loading="busy === 'save'" @click="savePreferences">Save policy</v-btn>
        <v-btn color="secondary" prepend-icon="mdi-play-circle-outline" :disabled="!selectedAvailable" :loading="busy === 'test'" @click="runSelfTest">Run provider self-test</v-btn>
        <v-btn variant="tonal" prepend-icon="mdi-delete-sweep-outline" :loading="busy === 'cache'" @click="clearCache">Clear provider cache</v-btn>
      </div>
    </v-card>
    ...
    <v-card v-if="selfTest" class="glass pa-6 mt-6">
      ...
      <v-row class="mt-2">
        <v-col cols="6" md="2"><div class="text-caption text-medium-emphasis">Registered</div><div class="font-weight-bold">{{ selfTest.registered ? 'Yes' : 'No' }}</div></v-col>
        <v-col cols="6" md="2"><div class="text-caption text-medium-emphasis">Session</div><div class="font-weight-bold">{{ selfTest.session_created ? 'Created' : 'Failed' }}</div></v-col>
        <v-col cols="6" md="2"><div class="text-caption text-medium-emphasis">Assigned nodes</div><div class="font-weight-bold">{{ selfTest.assigned_node_count }}</div></v-col>
        <v-col cols="6" md="2"><div class="text-caption text-medium-emphasis">CPU nodes</div><div class="font-weight-bold">{{ selfTest.cpu_assigned_node_count }}</div></v-col>
        <v-col cols="6" md="2"><div class="text-caption text-medium-emphasis">Cold run</div><div class="font-weight-bold">{{ selfTest.cold_inference_ms ?? '—' }} ms</div></v-col>
        <v-col cols="6" md="2"><div class="text-caption text-medium-emphasis">Warm average</div><div class="font-weight-bold">{{ selfTest.warm_inference_ms ?? '—' }} ms</div></v-col>
      </v-row>
      <v-alert v-if="selfTest.recommendation" :type="selfTest.passed ? 'success' : 'warning'" variant="tonal" class="mt-3">{{ selfTest.recommendation }}</v-alert>
      <v-alert v-if="selfTest.error" type="error" variant="tonal" class="mt-3">{{ selfTest.error.name }}: {{ selfTest.error.message }}</v-alert>
    </v-card>
