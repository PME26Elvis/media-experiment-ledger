...
const en = {
  ...
  hardware: { title: 'Know what will execute before starting a corpus', subtitle: 'Inspect the desktop GPU stack and packaged ONNX Runtime, select an explicit device policy, prove node assignment with a generated graph, and export redacted evidence.' },
  integrations: {
...
const zhTW = {
  ...
  hardware: { title: '在開始資料集工作前確認實際執行環境', subtitle: '檢查桌面 GPU 與封裝後 ONNX Runtime、設定明確裝置政策、以生成的圖驗證節點分配，並匯出已去識別證據。' },
  integrations: {
...
const zhCN = {
  ...
  hardware: { title: '在开始数据集任务前确认实际运行环境', subtitle: '检查桌面 GPU 与封装后的 ONNX Runtime、设置明确设备策略、用生成图验证节点分配，并导出去标识证据。' },
  integrations: {
    ...zhTW.integrations,
...
const ja = {
  ...
  hardware: { title: 'コーパス処理前に実行環境を確認', subtitle: 'デスクトップ GPU と同梱 ONNX Runtime を調査し、明示的なデバイスポリシーと生成グラフによるノード割当証拠を確認します。' },
  integrations: {
...
const ko = {
  ...
  hardware: { title: '코퍼스 작업 전에 실제 실행 환경 확인', subtitle: '데스크톱 GPU와 패키지 ONNX Runtime을 검사하고 명시적 장치 정책 및 생성 그래프의 노드 할당 증거를 확인합니다.' },
  integrations: {
    eyebrow: '예약, 동기화 및 게시', title: '통합 센터', subtitle: '로컬 프로젝트를 사용자 단위 OS 예약, 충돌 안전 폴더, 변경 불가 GitHub Release 및 제한된 WASM 어댑터에 연결합니다.',
    scheduler: 'OS 스케줄러', schedulerTitle: '예약된 Studio 작업', name: '예약 이름', enabled: '사용', cadence: '주기', intervalHours: '간격 시간', weekdays: '요일', jobKind: '작업 종류', jobTitle: '작업 제목', installSchedule: '예약 설치', noSchedules: '설치된 예약이 없습니다.', scheduleSaved: '예약을 설치했습니다.', scheduleQueued: '예약 작업이 대기열에 추가되었습니다.',
