...
export function engineRoot(): string {
  return app.isPackaged ? join(process.resourcesPath, 'engine') : join(app.getAppPath(), 'engine')
}

export async function engineReady(): Promise<boolean> {
  try { await access(join(engineRoot(), 'mel_engine', '__main__.py')); return true } catch { return false }
}
...
export function runEngine(payload: Record<string, unknown>, onEvent: (event: EngineEvent) => void, signal: AbortSignal): Promise<Record<string, unknown>> {
  return new Promise((resolve, reject) => {
    const child = spawn(pythonCommand(), ['-m', 'mel_engine'], {
      cwd: engineRoot(), env: { ...process.env, PYTHONPATH: engineRoot() }, stdio: ['pipe', 'pipe', 'pipe'], windowsHide: true, shell: false,
    })
    ...
    stdout.on('line', (line) => {
      try {
        ...
        if (event.type === 'error') reject(new Error(event.message ?? 'Engine error'))
